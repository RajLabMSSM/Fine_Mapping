...
[LDheatmap](https://cran.r-project.org/web/packages/LDheatmap/LDheatmap.pdf)
...
proxySNPs_LD <- function(){  
  # LD_proxy <- get_proxies(query = flankingSNPs$SNP[1:10], window_size = 10000, pop="CEU") 
  
  vcf <- proxysnps::get_vcf(chrom = flankingSNPs$CHR[1], 
                            start = min(flankingSNPs$POS), end = max(flankingSNPs$POS), 
                            pop = "CEU") 
...
* Cons
  + A maximum of 1Mb (1000000 bp) queries are allowed (according to documentation).
  + However, in practice only 100000 bp queries work (a whole order of magnitude less...)
