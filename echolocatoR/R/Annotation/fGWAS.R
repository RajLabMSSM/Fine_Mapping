...
fGWAS.line_plot <- function(RESULTS.DF, 
  ...
  annot.SORT <- annot.df %>%
    subset(SNP.Group == "Consensus") %>% 
    dplyr::group_by(Type) %>%
    arrange(Type, estimate, desc(AIC))
  ...
  # Plot
  ## Point w/ CIs
  point.plot <- ggplot(annot.df, aes(x=estimate, 
    ...
    labs(title = "fGWAS Results by SNP Group", x="Maximum-likelihood Estimate") + 
