...
multi_finemap_results_table <- function(multi_finemap_DT, 
  ...
  support_DT <- subset(multi_finemap_DT, Support >= minimum_support, 
                       select=c("Gene","SNP","CHR","POS","P",CS_cols,"Support")) %>% 
...
merge_finemapping_results <- function(finemap_results=NA, 
                                      minimum_support=1, 
  ...
  if(from_storage){
    ...
    multi_dirs <- list.files("./Data/GWAS/", pattern = "Multi-finemap_results.txt", recursive = T)
    dataset_names <- dirname(dirname(dirname(multi_dirs))) %>% unique() 
    # Loop through each dataset
    finemap_results <- lapply(dataset_names, function(dn){
      gene_dirs <- list.dirs( file.path("./Data/GWAS",dn), recursive = F )
      ...
      all_results <- lapply(gene_dirs, function(gd){
        ...
        return(multi_data)
      }) %>% data.table::rbindlist() # Bind genes
    }) %>% data.table::rbindlist() # Bind datasets    
  }
  
  # Merge list of results into one table
  merged_results <- lapply(unique(finemap_results$Dataset), function(n){ 
    multi_finemap_DT <- subset(finemap_results, Dataset==n) 
    CS_cols <- colnames(multi_finemap_DT)[endsWith(colnames(multi_finemap_DT), ".Credible_Set")]
    ...
    support_DT <- cbind(Dataset = n, support_DT) %>% arrange(Gene, desc(Support)) 
  }) %>% data.table::rbindlist() 
  
  # Annotate with haplorR
  if(haploreg_annotation){
    HR_query <- haploReg(merged_results)
    merged_results <- data.table:::merge.data.table(merged_results, HR_query, by.x = "SNP", by.y = "rsID", all = T)
  }
  if(biomart_annotation){
    biomart_query <- biomart_snp_info(merged_results) 
...
haploReg <- function(snp_list, verbose=T){
  printer("+ Gathering annotation data from HaploReg...", v=verbose)
  HR_query <-  haploR::queryHaploreg(query =snp_list, file = NULL, study = NULL, ldThresh = NA,
...
biomart_snp_info <- function(snp_list, reference_genome="grch37", verbose=T){
  ...
  biomart_query = getBM(attributes =  c('refsnp_id',
                                 'allele',
                                 'chr_name',
                                 'chrom_start',
                                 'chrom_end',
...
SNPs_by_mutation_type <- function(merged_results, mutation_type="missense_variant"){
