...
# 0. List available eQTL data
eQTLcatalogue.list_eQTL_datasets <- function(save_path="./echolocatoR/R/QTL_databases/",
                                             force_new=F,
                                             verbose=F){
...
# 1. Convert echoR to eQTLcatalogue format
eQTLcatalogue.convert_echoR_sumstats <- function(finemap_DT,
                                                 trait=NA){
  # c("variant_id", "chromosome", "base_pair_location", "trait",
  #   "p_value", "ci_lower", "ci_upper", "beta",
  #   "effect_allele", "other_allele", "effect_allele_frequency",
  #   "odds_ratio", "study_accession", "code")
  # quick_finemap()
  gwas_data <- finemap_DT %>% dplyr::select("Locus"=Gene,
                                            "variant_id"=SNP,
                                            "chromosome"=CHR,
...
eQTLcatalogue.fetch_tabix <- function(unique_id="Alasoo_2018.macrophage_naive",
                                      quant_method="ge",
                                      infer_region=T,
                                      gwas_data=NULL,
                                      chrom=NULL,
  ...
  # Get region
  if(infer_region){
    chrom <- unique(gwas_data$chromosome)[1]
    bp_lower <- min(gwas_data$base_pair_location)
    bp_upper <- max(gwas_data$base_pair_location)
  }
  region <- paste0(chrom,":",bp_lower,"-",bp_upper)
  # Get metadata
  ui <- unique_id
  qm <- quant_method
  meta <- eQTLcatalogue.list_eQTL_datasets(force_new = F)
  meta.sub <- subset(meta, unique_id==ui & quant_method==qm) %>% data.frame()
  # Run tabix
  ## Get the header (according to email with Kaur Alassoo)
  eQTLcatalogue.tabix_header <- function(tabix_path){
    eqtl_cat_path <- "./echolocatoR/annotations/eQTL_catalogue"
  ...
  header <- eQTLcatalogue.tabix_header(tabix_path = meta.sub$ftp_path)
  ...
  gwas_data <- data.table::data.table(gwas_data) %>%
    `colnames<-`(paste0(colnames(gwas_data),".GWAS"))

  # Merging and allele flipping
  gwas.qtl <- data.table:::merge.data.table(x = gwas_data,
                                            y = tbx,
                                            # all.x = T,
                                            by.x = c("chromosome.GWAS","base_pair_location.GWAS"), # effect_allele
                                            by.y = c("chromosome.QTL","position.QTL") ) %>%
    dplyr::mutate(effect.is.ref=ifelse(effect_allele.GWAS==ref.QTL,T,F),
                  effect.is.alt=ifelse(effect_allele.GWAS==alt.QTL,T,F) ) %>%
  ...
  printer("Cleaning up:",tbi)
  if(file.exists(tbi) & remove_tmp){file.remove(tbi)}
  return(gwas.qtl)
}
...
# ###
get_assocs_in_region <- function(unique_id="Alasoo_2018.macrophage_naive",
                                 quant_method="ge",
                                 infer_region=T,
                                 gwas_data=NULL,
                                 chrom=NULL,
  ...
  # Get region
  if(infer_region){
    chrom <- gsub("chr","",unique(gwas_data$chromosome)[1])
    bp_lower <- min(gwas_data$base_pair_location)
    bp_upper <- max(gwas_data$base_pair_location)
  }
  # Get metadata
  ...
  region_for_study_gene_data <- fetch_from_eqtl_cat_API(link = link,
                                                        is_gwas = is_gwas)
...
eQTLcatalogue.run <- function(loci,
                              qtl.datasets=NULL,
                              nThread=4){
  library("dplyr")
  library("ggplot2")
  ...
  loci <- dirname(list.files("./Data/GWAS/Nalls23andMe_2019",pattern = "*Nalls23andMe_2019_subset.tsv.gz", recursive = T)); nThread=4; qtl.datasets=c("Fairfax_2014","ROSMAP","Alasoo_2018","Nedelec_2016","BLUEPRINT", "HipSci.iPSC","Lepik_2017","BrainSeq","TwinsUK")
  # loc=loci[1]; qtl.id=qtl.datasets[1]
  meta <- eQTLcatalogue.list_eQTL_datasets(force_new = F)
  if(is.null(qtl.datasets)){
    printer("eQTL_catalogue:: Gathering data for all QTL Catalogue datasets...")
    qtl.datasets <- unique(meta$unique_id)
  }else {
    qtl.datasets <- grep(pattern = paste(qtl.datasets,collapse="|"),
                         x = meta$unique_id,
                         value = T,
                         ignore.case = T) %>% unique()
  }
  printer("eQTL_catalogue:: Querying",length(qtl.datasets),"QTL datasets...")

  # RUN
  GWAS.QTL_all <- lapply(qtl.datasets, function(qtl.id){
    GWAS.QTL <-data.table::data.table()
    try({
      message(qtl.id)
      GWAS.QTL <-  parallel::mclapply(loci, function(loc){
        printer(loc)
        gwas.qtl <- data.table::data.table()
        try({
          finemap_DT <- quick_finemap_soft(locus = loc)
          gwas_data <- eQTLcatalogue.convert_echoR_sumstats(finemap_DT, trait = "PD")

          gwas.qtl <- eQTLcatalogue.fetch_tabix(unique_id=qtl.id,
                                                quant_method="ge",
                                                infer_region=T,
                                                gwas_data=gwas_data,
                                                is_gwas=F,
      ...
      GWAS.QTL <- data.table::data.table(qtl.id=qtl.id, GWAS.QTL)
    })
    return(GWAS.QTL)
  ...
  # Get QTL gene names
  gene_dict <- ensembl_to_hgnc(ensembl_ids = GWAS.QTL_all$molecular_trait_object_id.QTL)
  ...
  missing.qtls <- qtl.datasets[!qtl.datasets %in% unique(GWAS.QTL_filt$qtl.id)]
  printer("eQTL_Catalogue: QTL datasets with no hits/failed to pull data:")
  printer(paste(missing.qtls, collapse=", "))
  data.table::fwrite(GWAS.QTL_filt,
                     file = file.path("./Data/GWAS/Nalls23andMe_2019/_genome_wide/","Nalls23andMe_2019.eQTL_Catalogue.tsv.gz"),
                     nThread = 4)
...
get_significant_assocs_of_var <- function(study_ids,
  ...
  for(study_name in study_ids){
    ...
    ge_study_query_gwas_lead <- paste0("https://www.ebi.ac.uk/eqtl/api/associations?variant_id=", variant_id,
                                       ...
                                       "&study=", study_name,
                                       "&quant_method=", quant_method)
    fetched_df <- eQTLcatalogue.fetch_data(link = ge_study_query_gwas_lead)
...
# bp_lower: 45980000
# bp_upper: 46200000
eQTLcatalogue.full_example <- function(){
