...
hgnc_to_ensembl <- function(gene_symbols){ 
   symbols <- mapIds(org.Hs.eg.db::org.Hs.eg.db, 
                    keys = gene_symbols, 
                    keytype = "SYMBOL", 
                    column = "ENSEMBL")
  return(symbols)
...
eQTLcatalogue.run <- function(){
  ...
  eQTLcatalogue.list_eQTL_datasets <- function(save_path="./",
    ...
    if(file.exists(meta.path) & force_new==F){
      ...
      if(save_path!=F){ 
        ...
        data.table::fwrite(meta, meta.path, nThread = 4)
      }
    } 
    printer(length(unique(meta$study)),"unique eQTL datasets:")
  ...
  eQTLcatalogue.convert_echoR_sumstats <- function(finemap_DT,
    ...
    # quick_finemap()
    gwas_data <- finemap_DT %>% dplyr::select("variant_id"=SNP,
                                              "chromosome"=CHR,
  ...
  # RUN
  meta <- eQTLcatalogue.list_eQTL_datasets(force_new = T)
  finemap_DT <- quick_finemap_soft()
  gwas_data <- eQTLcatalogue.convert_echoR_sumstats(finemap_DT, trait = "PD")
  lead_var_gwas = dplyr::arrange(subset(gwas_data, MAF>0.05), p_value)[1,]
