...
ABF <- function(subset_DT, PP_threshold=.5){
  ...
  finemap_DT <- data.table:::merge.data.table(x=data.table::data.table(subset_DT), 
                                              y=data.table::data.table(subset(finemap_DT, select=c("SNP","Probability","Credible_Set")) ), 
                                              on="SNP")
  finemap_DT <- finemap_DT %>% arrange(desc(Probability))
  return(finemap_DT)
} 
  ...
  for(i in 1:length(finemap_method_list)){  
    ...
    tryCatch(
      ...
      DT <- finemap_method_handler(fullSS_path = fullSS_path,
                                   ...
                                   A2_col = A2_col),
      # WARNING 
      warning = function(merged_DT){ printer("WARNING");},
      # ERROR
      error = function(merged_DT){ printer("ERROR:: Could not identify Credible Set."); 
        }
      )  
