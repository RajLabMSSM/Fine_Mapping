...
# # Filter loci
# finished_loci <- list.files("./Data/GWAS/Nalls23andMe_2019",pattern = ".npz$", 
#            recursive = T, full.names = T) %>% dirname() %>% dirname() %>% basename()
# loci <- loci[!loci%in% finished_loci]
# Limit LRRK2
 

finemap_results[[dataset_name]] <- finemap_loci(top_SNPs,
                                               loci = "SLC2A13",#unique(c("LRRK2", loci)),
