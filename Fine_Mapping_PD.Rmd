...
### Gather QTL data from eQTL Catalogue

```{r Gather QTL data from eQTL Catalogue}
if(startsWith(getwd(),"/sc/")){
...
# sumstats_paths <- sumstats_paths[startsWith(basename(dirname(dirname(sumstats_paths))), "LRRK2")] 
...
coloc_qtls <- data.table::fread("./Data/GWAS/Nalls23andMe_2019/_genome_wide/COLOC/eQTL_Catalogue.coloc.tsv.gz")
coloc_qtls <- subset(coloc_qtls, !Locus.GWAS %in% c(no_no_loci, problem_children))

# PLOT
# Get colocalized eGenes for each locus-qtl.id comparison 
