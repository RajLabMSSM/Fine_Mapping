...
nott_overlap <- cbind(data.frame(gr.finemap[overlap@to]),
                      data.frame(gr.peaks[overlap@from]))[,c("SNP",colnames(gr.peaks@elementMetadata))]
...
consensus_melt <-  setDT(merged_annot)[, .(Count = uniqueN(SNP[topConsensus == T])), 
...
gg_consensus_annot <- ggplot(data=consensus_melt, aes(x=Annotation, y=Locus, fill=Count)) +
   ...
   scale_fill_gradient(na.value = "white", low = scales::alpha("blue",.7), high = scales::alpha("red",.7)) +
...
```{r QTLs}
qtl_DT <- top_finemapped_loci(dataset="./Data/GWAS/Nalls23andMe_2019",
                                save_results=T,
                                biomart=F,
