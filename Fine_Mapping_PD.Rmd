...
sumstats_paths <- list.files("./Data/GWAS/Nalls23andMe_2019",
                             ...
                             recursive = T, full.names = T)
# meta <- catalogueR.list_eQTL_datasets(save_path = F)
gwas.qtl_paths <- catalogueR.run(
                           sumstats_paths = sumstats_paths,
                           # loci_names = basename(dirname(sumstats_paths)), 
                           # qtl_search = "ROSMAP",
                           ...
                           # use_tabix = F,
                           force_new_subset = F,
                           # output_path = "./Data/GWAS/Nalls23andMe_2019/_genome_wide/eQTL_catalogue"
                           output_path = "/Volumes/Scizor/eQTL_catalogue/Nalls23andMe_2019"
                           )
# Use saved
# qtl.dat <- data.table::fread(file.path("./Data/GWAS/Nalls23andMe_2019/_genome_wide/eQTL_catalogue",
#                                        "Nalls23andMe_2019.eQTL_Catalogue.tsv.gz"),  nThread = 4)
gwas_data <- merge_finemapping_results(minimum_support=0,
                                      include_leadSNPs=T,
                                      dataset = "./Data/GWAS/Nalls23andMe_2019",
                                      xlsx_path=F,
                                      from_storage=T,
                                      ...
                                      biomart_annotation=F, 
                                      verbose=F)
gwas_data <- gwas_data %>% dplyr::rename(Locus=Gene) 
no_no_loci <- c("HLA-DRB5","MAPT","ATG14","SP1","LMNB1","ATP6V0A1")
gwas_data <- subset(gwas_data, !(Locus %in% no_no_loci))


qtl.dat <- rbind.file.list(file.list=gwas.qtl_paths, nCores = 4 )  
coloc_qtls <- catalogueR.run_coloc(qtl.dat = qtl.dat,
                                   gwas_data = gwas_data) 


minPP4 <- .5
coloc_plot <- subset(coloc_qtls, !is.na(Locus.GWAS)) %>% dplyr::mutate(Colocalized = ifelse(PP.H4>=minPP4,PP.H4,NA))
# coloc_plot[is.na(coloc_plot),"PP.H4"] <- F

ggplot(data=coloc_plot, aes(x=Locus.GWAS, y=qtl.id, fill=Colocalized)) + 
  geom_raster() +  
  # scale_fill_continuous(limits=c(minPP4,1)) +
  scale_fill_gradient(na.value = "transparent", low = scales::alpha("blue",.7), high = scales::alpha("red",.7)) +
  scale_x_discrete(drop=F) +
  # scale_fill_manual(limits=c(0,1), palette="Spectral") +
  facet_grid(facets = .~eGene) +
  theme_classic() 
  # theme(strip.text.x = element_text(margin =))
...
```{r QTLs}  
gwas.qtl <- data.table:::merge.data.table(x = merged_DT %>% dplyr::select(Locus, CHR, POS, SNP, P, leadSNP, Consensus_SNP, Support),
                                          y = qtl.dat,# subset(qtl.dat, pvalue.QTL<5e-8), 
                                          all.x = T,
                                          by.x = c("Locus","CHR","POS"),
                                          by.y = c("Locus.GWAS","chromosome.GWAS","base_pair_location.GWAS") 
                                          ) %>% 
  dplyr::mutate(UCS = Support>0)

qtl.thresh = .05
plot.subset <- subset(gwas.qtl, pvalue.QTL<qtl.thresh &
                      Locus %in% c("LRRK2","BST1","MED12L","CHRNB1","BIN3")) %>% 
